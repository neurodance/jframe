...
export async function buildApp() {
  ...
  app.addHook('preHandler', async (req, reply) => {
    ...
    if (provided !== requiredKey) {
      reply.code(401).send({ error: { code: 'unauthorized', message: 'Invalid or missing API key', correlationId: (req as any).correlationId } })
    }
  })

  // DB state
  let orgs = [] as Array<ReturnType<typeof OrgSchema.parse>>
  let projects = [] as Array<ReturnType<typeof ProjectSchema.parse>>
  let jobs = [] as Array<ReturnType<typeof JobSchema.parse>>
  let runs = [] as Array<ReturnType<typeof RunSchema.parse>>
  ...
  function broadcast(evt: SseEvent) { bus.emit('broadcast', evt) }
  ...
  app.post('/v1/orgs', async (req, reply) => {
    ...
    if (!parsed.success) { reply.code(400).send({ error: { code: 'bad_request', message: 'Invalid org payload', details: parsed.error.flatten(), correlationId: (req as any).correlationId } }); return }
  ...
  app.post('/v1/projects', async (req, reply) => {
    ...
    if (!parsed.success) { reply.code(400).send({ error: { code: 'bad_request', message: 'Invalid project payload', details: parsed.error.flatten(), correlationId: (req as any).correlationId } }); return }
    let { name, orgId } = parsed.data
    if (!orgId) {
      if (orgs.length === 1) orgId = orgs[0].id
      else {
        let devOrg = orgs.find(o => o.id === 'org-dev')
        if (!devOrg) { devOrg = OrgSchema.parse({ id: 'org-dev', name: 'Development Org', createdAt: new Date().toISOString() }) ; orgs.push(devOrg) }
        orgId = devOrg.id
      }
    }
  ...
  app.get('/v1/projects/:id', async (req, reply) => {
    await loadDb(); const id = (req.params as any).id as string; const item = projects.find(p => p.id === id)
    if (!item) { reply.code(404).send({ error: { code: 'not_found', message: 'Project not found', correlationId: (req as any).correlationId } }); return }
  ...
  app.post('/v1/jobs', async (req, reply) => {
    ...
    if (!parsed.success) { reply.code(400).send({ error: { code: 'bad_request', message: 'Invalid job payload', details: parsed.error.flatten(), correlationId: (req as any).correlationId } }); return }
  ...
  app.get('/v1/jobs/:id', async (req, reply) => {
    await loadDb(); const id = (req.params as any).id as string; const item = jobs.find(j => j.id === id)
    if (!item) { reply.code(404).send({ error: { code: 'not_found', message: 'Job not found', correlationId: (req as any).correlationId } }); return }
    reply.send(JobSchema.parse(item))
  })
  app.get('/v1/jobs/:id/runs', async (req, reply) => { await loadDb(); const id = (req.params as any).id as string; reply.send({ items: runs.filter(r => r.jobId === id).map(r => RunSchema.parse(r)), next: null }) })
  app.post('/v1/jobs/:id/runs', async (req, reply) => {
    await loadDb(); const id = (req.params as any).id as string
    const parsed = RunCreateSchema.safeParse(req.body ?? {})
    if (!parsed.success) { reply.code(400).send({ error: { code: 'bad_request', message: 'Invalid run payload', details: parsed.error.flatten(), correlationId: (req as any).correlationId } }); return }
    const job = jobs.find(j => j.id === id)
    if (!job) { reply.code(404).send({ error: { code: 'not_found', message: 'Job not found', correlationId: (req as any).correlationId } }); return }
    ...
    if (run.status === 'succeeded' || run.status === 'failed') bus.emit('broadcast', { event: 'run.completed', data: { id: run.id, jobId: run.jobId, status: run.status, completedAt: run.createdAt } })
  ...
  app.get('/v1/events/stream', async (req, reply) => {
    ...
    const onBroadcast = (evt: SseEvent) => {
      try { reply.raw.write(`event: ${evt.event}\ndata: ${JSON.stringify(evt.data)}\n\n`) } catch {}
    }
    bus.on('broadcast', onBroadcast)
    const hb = setInterval(() => { try { reply.raw.write(`event: heartbeat\ndata: ${JSON.stringify({ at: new Date().toISOString() })}\n\n`) } catch {} }, 15000)
